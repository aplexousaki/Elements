...
import sdl2
import sdl2.ext
from sdl2.keycode import SDLK_ESCAPE, SDLK_w, SDLK_f, SDLK_a
...
class SDL2Window(RenderWindow):
    ...
    def init(self):
        ...
        self._gWindow = sdl2.SDL_CreateWindow(
            self._windowTitle.encode(),
        ...
        if self._gContext is None:
            print(
                "OpenGL Context could not be created! SDL Error: ", sdl2.SDL_GetError()
            )
            exit(1)
...
class ImGUIDecorator(RenderDecorator):
    ...
    def init(self):
        ...
        #
        self._updateWireframe = Elements.pyECSS.Event.Event(
            name="OnUpdateWireframe", id=201, value=None
        )
        if self._wrapeeWindow.eventManager is not None:
            self._wrapeeWindow.eventManager._events[
                self._updateWireframe.name
            ] = self._updateWireframe
            self._wrapeeWindow.eventManager._publishers[
                self._updateWireframe.name
            ] = self

        self._updateCamera = Elements.pyECSS.Event.Event(
            name="OnUpdateCamera", id=300, value=None
        )
        if self._wrapeeWindow.eventManager is not None:
            self._wrapeeWindow.eventManager._events[
                self._updateCamera.name
            ] = self._updateCamera
    ...
    def event_input_process(self, running=True):
        ...
        for event in events:
            if event.type == sdl2.SDL_KEYDOWN:
                if event.key.keysym.sym == sdl2.SDLK_ESCAPE:
                    running = False
                ##################  toggle the wireframe using the F button  #############################
                elif event.key.keysym.sym == sdl2.SDLK_f:
                    self.toggle_Wireframe()
                ##################  increase eye[0] using the D button  #############################
                elif event.key.keysym.sym == sdl2.SDLK_d:
                    self.moveEye(0, 1)
                ##################  decrease eye[0] using the A button  #############################
                elif event.key.keysym.sym == sdl2.SDLK_a:
                    self.moveEye(0, 0)
                ##################  increase eye[1] using the W button  #############################
                elif event.key.keysym.sym == sdl2.SDLK_w:
                    self.moveEye(1, 1)
                ##################  decrease eye[1] using the S button  #############################
                elif event.key.keysym.sym == sdl2.SDLK_s:
                    self.moveEye(1, 0)
                ##################  decrease eye[1] using the Q button  #############################
                elif event.key.keysym.sym == sdl2.SDLK_q:
                    self.moveEye(2, 1)
                ##################  increase eye[1] using the E button  #############################
                elif event.key.keysym.sym == sdl2.SDLK_e:
                    self.moveEye(2, 0)

                ##################  increase eye[0] using the U button  #############################
                elif event.key.keysym.sym == sdl2.SDLK_u:
                    self.moveTarget(0, 1)
                ##################  decrease eye[0] using the O button  #############################
                elif event.key.keysym.sym == sdl2.SDLK_o:
                    self.moveTarget(0, 0)
                ##################  increase eye[1] using the I button  #############################
                elif event.key.keysym.sym == sdl2.SDLK_i:
                    self.moveTarget(1, 1)
                ##################  decrease eye[1] using the K button  #############################
                elif event.key.keysym.sym == sdl2.SDLK_k:
                    self.moveTarget(1, 0)
                ##################  decrease eye[1] using the J button  #############################
                elif event.key.keysym.sym == sdl2.SDLK_j:
                    self.moveTarget(2, 1)
                ##################  increase eye[1] using the L button  #############################
                ...
                ##################  decrease eye[1] using the K button  #############################
                    ...
                    self.moveUp(1, 0)
                ##################  decrease eye[1] using the J button  #############################
                elif event.key.keysym.sym == sdl2.SDLK_8:
                    self.moveUp(2, 1)
                ##################  increase eye[1] using the L button  #############################
                elif event.key.keysym.sym == sdl2.SDLK_9:
                    self.moveUp(2, 0)
    ...
    def extra(self):
        ...
        self._changed, self._colorEditor = imgui.color_edit3(
            "Color edit", *self._colorEditor
        )
        if self._changed:
        ...
        self._changed, self._eye = imgui.drag_float3(
            "Eye",
            *self._eye,
            change_speed=0.01,
            min_value=-10,
        ...
        self._changed, self._target = imgui.drag_float3(
            "Target",
            *self._target,
            change_speed=0.01,
            min_value=-10,
        ...
        self._changed, self._up = imgui.drag_float3(
...
class ImGUIecssDecorator(ImGUIDecorator):
    ...
    def __init__(self, wrapee: RenderWindow, imguiContext=None):
        ...
        self.selected = None
        # Selected should be a component
        ...
        if component._children is not None:
            ...
            while not done_traversing:
                try:
                    ...
                    if imgui.tree_node(
                        comp.name + "##" + str(comp.id), imgui.TREE_NODE_OPEN_ON_ARROW
                    ):
                        ...
                        if selected:
                            if (
                                comp != self.selected
                            ):  # First time selecting it. Set trs values to GUI;
                                self.selected = comp
                    ...
                    self.drawNode(
                        comp
                    )  # recursive call of this method to traverse hierarchy
